...
import { Percent, Receipt, Plus, Trash2, CheckCircle2, Save, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function ChargesList() {
  ...
  const [editFields, setEditFields] = useState<{name: string, value: string, type: 'percentage' | 'fixed', splitMode?: 'proportional' | 'equal'} | null>(null);
  ...
  const handleStartEdit = (charge: any) => {
    ...
    setEditFields({
      ...
      type: charge.type,
      splitMode: charge.splitMode || 'proportional'
    });
  };
  ...
  const handleAddExtra = () => {
    if (extraName.trim() && extraValue) {
      dispatch(addExtraCharge({
        ...
        type: extraType,
        splitMode: extraType === 'fixed' ? extraSplitMode : undefined
      }));
      setExtraName('');
      setExtraValue('');
    }
  ...
  return (
    <div className="card">
      ...
      >
        <h2 className="section-title" style={{ marginBottom: 0 }}><Receipt size={24} /> {t('taxesCharges')}</h2>
        ...
        <>
          ...
          <div style={{ paddingTop: '1rem' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '1.5rem' }}>
              ...
              {extraType === 'fixed' && (
                <div style={{ flex: '1 1 145px' }}>
                  <select value={extraSplitMode} onChange={(e) => setExtraSplitMode(e.target.value as any)}>
                  ...
                  {editingId === charge.id ? (
                    ...
                    <>
                      <span style={{ fontWeight: 600, cursor: 'pointer' }} onClick={() => handleStartEdit(charge)}>
                        {charge.name} {charge.type === 'fixed' && `(${charge.splitMode === 'equal' ? t('equal') : t('proportional')})`}
                      </span>
                      <div className="flex">
