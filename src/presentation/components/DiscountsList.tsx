...
import { Tag, Plus, Trash2, Info, Check, Save, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function DiscountsList() {
  ...
  const handleStartEdit = (discount: any) => {
    ...
    setEditFields({
      ...
      splitMode: discount.splitMode || 'proportional'
    });
  };
  ...
  const handleSaveEdit = () => {
    if (editingId && editFields) {
      dispatch(updateDiscount({
        ...
        splitMode: editFields.type === 'fixed' ? editFields.splitMode : undefined
      }));
      setEditingId(null);
      setEditFields(null);
    }
  ...
  const handleAddDiscount = () => {
    if (name.trim() && value) {
      dispatch(addDiscount({
        ...
        splitMode: type === 'fixed' ? splitMode : undefined
      }));
      setName('');
      setValue('');
      setMinPurchase('');
  ...
  return (
    <div className="card">
      <div 
        ...
        style={{ cursor: 'pointer', userSelect: 'none', marginBottom: isCollapsed ? '0' : '2rem' }}
      >
        <h2 className="section-title" style={{ marginBottom: 0 }}><Tag size={24} /> {t('discounts')}</h2>
        ...
        <>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2rem' }}>
            <div className="flex" style={{ flexWrap: 'wrap' }}>
              ...
              {type === 'fixed' && (
                <div style={{ flex: '1 1 145px' }}>
                  <label className="input-label">{t('splitMode')}</label>
                  <select value={splitMode} onChange={(e) => setSplitMode(e.target.value as any)}>
                ...
                <div key={discount.id} className="flex-between" style={{ padding: '1rem', background: 'var(--card-bg)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)', position: 'relative' }}>
                  {editingId === discount.id ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', width: '100%' }}>
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
                        {editFields?.type === 'fixed' ? (
                          <>
                            <select
                              value={editFields?.splitMode || 'proportional'}
                              onChange={(e) => setEditFields(prev => prev ? { ...prev, splitMode: e.target.value as any } : null)}
                            ...
                            <input
                              ...
                              style={{ flex: '1 1 150px', minWidth: '120px', fontSize: '0.875rem' }}
                            />
                          </>
                        ) : (
                          <>
                        ...
                        <button className="btn-primary" onClick={handleSaveEdit} style={{ height: '40px', width: '40px', borderRadius: '50%', padding: 0, flexShrink: 0, marginLeft: 'auto' }}>
                          <Save size={20} />
                        </button>
                      </div>
                    </div>
                    ...
                    <>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem', cursor: 'pointer', flex: 1, minWidth: 0 }} onClick={() => handleStartEdit(discount)}>
                        <span style={{ fontWeight: 700, fontSize: '1.1rem', color: 'var(--text-main)', display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {discount.name} {discount.type === 'fixed' && `(${discount.splitMode === 'equal' ? t('equal') : t('proportional')})`}
                        </span>
